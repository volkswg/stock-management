...
import { SALES_PROFIT_COST_OVERRIDE_HEADERS } from "./const";

export class SalesProfitCostOverridesSheet
  extends BaseGoogleSheet
  implements IGoogleRowsSheet
{
  async readRows(
    range = `A:${lastColumnLetter(
      SALES_PROFIT_COST_OVERRIDE_HEADERS.length - 1,
    )}`,
